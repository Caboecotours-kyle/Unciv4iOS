...
class SafeAreaViewportTest {
    ...
    @Test fun safeControlsProjectToTheSamePixelsInBothModes() {
        for ((width, height, insets) in listOf(
            ...
            assertEquals(Rectangle(0f, 0f, viewport.worldWidth, viewport.worldHeight), viewport.safeAreaBoundsInWorld)
    ...
    @Test fun fullScreenLayoutUsesTheEntireScreenAndReportsSafeBounds() {
        val width = 852
        val height = 393
        val insets = SafeInsets(59, 0, 0, 21)
        screen(width, height)
        val viewport = SafeAreaViewport(500f, useFullScreenLayout = true)
